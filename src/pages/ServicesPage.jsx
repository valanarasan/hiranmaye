            ...
            return (
              <Tilt3DCard key={srv.id} maxTilt={8}>
                <div
                  ...
                  className="glass-dark glass-dark-hover rounded-2xl p-5 sm:p-6 border-white/[0.04] cursor-pointer group h-full flex flex-col justify-between min-h-[180px]"
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <div className="w-10 h-10 rounded-xl bg-white/[0.04] text-zinc-500 flex items-center justify-center group-hover:bg-purple-500/20 group-hover:text-purple-400 transition-all">
                    ...
                    <h3 className="text-sm sm:text-base font-bold text-white group-hover:text-purple-300 transition-colors leading-tight">
                      {srv.title}
                    </h3>
                  </div>

                  <div className="flex items-center space-x-1 text-[10px] text-zinc-600 mt-3 group-hover:text-purple-400/60 transition-colors">
                    <span>Details</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
          <div className="relative w-full max-w-2xl max-h-[85vh] glass-dark rounded-2xl p-6 sm:p-8 border-purple-500/20 shadow-2xl overflow-y-auto">
            ...
            <button
              ...
              className="absolute top-4 right-4 p-2 rounded-full bg-white/[0.06] text-zinc-400 hover:text-white border border-white/[0.06] cursor-pointer z-10"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2 pr-10">
                <div className="w-12 h-12 rounded-xl bg-purple-500/15 text-purple-400 flex items-center justify-center mb-3">
                  {(() => { const IC = iconMap[expandedService.id] || Target; return <IC className="w-6 h-6" />; })()}
                </div>
                <h2 className="text-2xl sm:text-3xl font-bold text-white">{expandedService.title}</h2>
              ...
              {expandedService.pillars && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {expandedService.pillars.map((pil) => (
                    <div key={pil.name} className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] space-y-1">
                      <h4 className="text-xs font-bold text-purple-300">{pil.name}</h4>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {expandedService.offers.map((offer) => (
                    <div key={offer} className="flex items-center space-x-2 text-xs text-zinc-400">
